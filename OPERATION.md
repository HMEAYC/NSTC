...
│  ┌──────────────────┐       ┌──────────────────┐          │
│  │  ESP32-C3 腰帶    │       │  教師端攝影機      │          │
│  │  MPU6500 IMU     │       │  YOLO + MediaPipe │          │
│  │  50Hz WebSocket  │       │  即時姿勢估計     │          │
│  └───────┬──────────┘       └────────┬─────────┘          │
│          │ WS JSON                   │ WS binary (JPEG)    │
│          ▼                           ▼                     │
│  ┌──────────────────────────────────────────────┐          │
│  │           FastAPI 後端 (:8000)                 │          │
│  │                                                │          │
...
| 韌體 OTA | HTTPS（GitHub Pages）+ SHA-256 驗證 | 無線韌體更新（每 24 小時檢查） |

### 認證方式

| 方式 | 用途 | 說明 |
|------|------|------|
| JWT Token | Dashboard 使用者 | `POST /api/auth/login` 取得 Bearer token |
| API Key | ESP32 裝置 | `X-API-Key` header 認證（裝置註冊、WiFi 設定、Session 查詢） |
...
ESP32 會在連線後定期向 `/api/config/wifi?include_password=true&device_id=...` 讀取設定（使用 API Key 認證），並在 NVS 中保留最新值。
...
- WS 連線狀態指示燈（綠/黃/紅）
- Session ID
- **攝影機預覽**：開啟攝影機後顯示即時影像 + COCO-17 姿勢骨架疊加 + CV 指標徽章
...
3. 點擊「開啟攝影機」→ 瀏覽器請求攝影機授權 → 自動開始 10fps JPEG 傳送
4. 後端 YOLO + MediaPipe 即時分析 → 前端顯示骨架疊加 + 6 項 CV 指標
5. 若裝置未被指派 session，會連到 `/ws/default`，此時可用 `/dashboard/live/default` 檢視
6. 最多保留 200 筆資料點，自動滑動
...
**🟡 需音樂參考（已可用 — 上傳音樂檔或設定 BPM 後即計算）：**

| 指標 | 說明 | 現狀 |
|------|------|------|
| 🎵 節奏同步誤差 | FFT 相位匹配 vs 音樂拍點 | ✅ 已實作，見 `POST /api/sessions/{id}/music`（§6.4/§8.1） |
| 🧊 凍結反應/穩定度 | 音樂停止瞬間反應時間 + 穩定度 | ✅ 已實作，音樂上傳自動偵測停止時間點 |

**🔴 需攝影機資料（YOLO + MediaPipe 即時分析）：**

| 指標 | 說明 | 計算方式 |
|------|------|---------|
| 👥 團體投入度 | 活躍比例（>5px移動） | 最近 30 幀中活躍幀比例 |
| 📐 隊形穩定度 | 幾何分類信心均值 | 質心跨幀標準差 |
| 🗺️ 空間利用率 | 3×3 熱區分布離散度 | 3×3 網格覆蓋率 |
| 🦶 步態對稱性 | 左右腳支撐期比對 | 左右膝-踝 y 位移差 |
| 🧘 平衡搖擺面積 | 質心投影軌跡 | 質心軌跡 bounding box 面積 |
| 🤝 上下肢協調 | PLV 相位鎖定值 | 上肢與下肢動作相關性 |
...
- ESP32 開機會自動註冊（含 WiFi 資訊），並每 2 分鐘發送心跳保持上線狀態

**網路掃描（super_admin 專用）：**
- 點擊「掃描網路」按鈕，系統透過 ARP 表掃描區域網路
- 過濾 Espressif OUI 前綴（23 個已知前綴），排除已註冊裝置
- 顯示未註冊的 ESP32，點擊「註冊」即可加入
...
| Method | Path | 說明 |
|--------|------|------|
| `GET` | `/api/config/wifi` | 讀取 WiFi 設定（支援 `device_id` 參數，預設只回傳 `ssid` 與 `updated_at`）；支援 API Key 認證 |
| `PUT` | `/api/config/wifi` | 更新 WiFi 設定（body: `ssid`, `password?`, `device_id?`） |
| `GET` | `/api/config/session?device_id=<device_id>` | 查詢裝置被指派到的 session（支援 API Key 認證） |
...
| Method | Path | 說明 |
|--------|------|------|
| `GET` | `/api/orgs/{orgId}/users` | 列出組織使用者（含家長綁定幼兒列表） |
| `POST` | `/api/orgs/{orgId}/invite` | 邀請使用者加入組織（body: `email`, `role`） |
| `PUT` | `/api/users/{id}` | 編輯使用者（body: `is_active?`, `display_name?`, `password?`, `role?`） |
| `POST` | `/api/users/{id}/resend-invite` | 重新發送邀請 Email |

> **角色驗證**：`role` 欄位僅允許 `teacher`、`org_admin`、`parent` 三個值。`super_admin` 只能由 super_admin 指派。
...
**Dashboard → Server（攝影機 binary）：**
- 二進制 JPEG 幀（640×480, q=0.6, ~10fps）
- 控制訊息（JSON text）：
  - `{"type": "camera_start", "fps": 10}` — 開始攝影機串流
  - `{"type": "camera_stop"}` — 停止攝影機串流

**Server → Dashboard（姿勢結果 JSON）：**
```json
{
  "type": "pose_update",
  "poses": [
  ...
  "cv_metrics": {
    "engagement": 0.85,
    "formation_stability": 0.72,
    "spatial_utilization": 0.67,
    "gait_symmetry": 0.78,
    "balance_sway": 0.81,
    "limb_coordination": 0.63
  }
}
```

**Server → Dashboard（其他訊息）：**
- `{"type": "rhythm_update", "sync_rate": 0.75, ...}` — 節奏同步結果
- `{"type": "freeze_update", "stop_time": 5.0, ...}` — 凍結反應結果
- `{"type": "music", ...}` — 音樂資訊
- `{"type": "camera_status", "status": "streaming"}` — 攝影機狀態
...
```bash
# 檢查 I2C 接線（預設 GPIO6=SDA, GPIO7=SCL，可透過 Kconfig 調整）
# SDA → GPIO6 (CONFIG_HMEAYC_I2C_SDA_PIN)
# SCL → GPIO7 (CONFIG_HMEAYC_I2C_SCL_PIN)
# VCC → 3.3V, GND → GND
...
2. 點擊「掃描網路」按鈕（super_admin 專用）
3. 系統透過 ARP 表掃描區域網路，過濾 Espressif OUI 前綴
4. 顯示未註冊的 ESP32 裝置，點擊「註冊」即可加入
