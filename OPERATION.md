...
make lint-dashboard     # tsc --noEmit
```
...
SDK config 中設定（`firmware/sdkconfig.defaults`）：

```
CONFIG_WIFI_SSID="chen"
CONFIG_WIFI_PASSWORD="12345678"
CONFIG_WEBSOCKET_URI="ws://192.168.1.105:8080/ws/default"
```
...
**自動註冊：** ESP32 開機後，韌體中的 `device_id` 會自動透過 `POST /api/devices` 向後端註冊。可在 Dashboard 裝置列表看到。
...
grep -E "WIFI_SSID|WIFI_PASSWORD" firmware/sdkconfig.defaults
...
# sdkconfig.defaults 中：CONFIG_WEBSOCKET_URI="ws://{BACKEND_IP}:8080/ws/default"
...
# 檢查 ESP32 是否有發送 POST /api/devices（需在韌體中加入）
```
