...
from app.paths import metrics_dir, repo_root
from app.tracking.longitudinal import append_session, load_sessions, sessions_jsonl_path
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="批次匯入 reports/metrics/*_metrics.json 至跨影片累積檔 sessions.jsonl（依 import_metrics 去重）"
    )
    ap.add_argument(
        "--memory",
        type=Path,
        default=None,
        help="掃描目錄（預設專案 reports/metrics/）",
    )
