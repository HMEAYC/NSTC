...
| **WiFi 配網方式** | SoftAP Captive Portal（連線 `HMEAYC-Setup` → DNS 自動導向設定頁 → 輸入 WiFi） |
...
| SoftAP Captive Portal 配網（含 DNS 自動導向 + iOS/Android 偵測） | ✅ 完成 |
| 裝置心跳（2 分鐘） | ✅ 完成 |
...
| Dashboard 機構篩選（帳號/班級/課程/裝置頁面） | ✅ 完成 |
| 學號班級代號前綴（自動加前綴/顯示時剝除） | ✅ 完成 |
| Dashboard 20 個頁面 | ✅ 完成 |
...
| 角色 | 權限範圍 |
|------|---------|
| **super_admin** | 全域管理，可檢視/管理所有組織（帳號/班級/課程/裝置頁面均有機構篩選下拉選單） |
...
| `GET` | `/api/classes` | 班級列表（role-based：super_admin 全部/org_admin 機構內/teacher 所教班級） |
| `GET` | `/api/users` | 使用者列表（role-based 過濾） |
| `GET` | `/api/sessions` | 課程列表（role-based 過濾，teacher 僅顯示自己的課程） |
| `POST` | `/api/orgs/{orgId}/invite` | 邀請教師（super_admin 需先選擇機構） |
| `PUT` | `/api/children/{childId}` | 更新幼兒（學號自動加班級代號前綴） |
...
- **Device** — ESP32 腰帶註冊（device_id, name, firmware_version, battery_level, mac_address, ip_address, wifi_ssid, wifi_rssi, status, active_session_id）
- **Child** — 學員資料（name, student_id [DB 存 `{班級代號}-{學號}`，API 回傳剝除前綴], notes）
