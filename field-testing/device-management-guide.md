...
# 2. 啟動後端
cd backend && python3 -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8080

# 3. 啟動 Dashboard（另一個 terminal）
cd dashboard && npm run dev
```

驗證：`curl http://localhost:8080/health` → `{"status":"ok"}`

### Step 2: 註冊 ESP32 腰帶（自動 / 手動）

**自動方式：** ESP32 開機後會透過 WiFi 連線，由後端 `POST /api/devices` 自動註冊。  
Device ID 由韌體中的 `device_id` 定義。
...
CHILDREN=$(curl -s http://localhost:8080/api/children)
...
```bash
curl -s http://localhost:8080/api/sessions/{SESSION_ID}/assignments | python3 -m json.tool
```
...
A: 檢查 ESP32 是否成功發送 `POST /api/devices`。可手動呼叫測試。
